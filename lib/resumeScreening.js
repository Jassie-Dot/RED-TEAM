...
import { callGroqJson, hasGroqCredentials } from "./groqClient.js";

const LEVEL_SCORE = { Foundational: 35, Intermediate: 62, Advanced: 82, Expert: 94 };
const LEVEL_RANK = { Foundational: 1, Intermediate: 2, Advanced: 3, Expert: 4 };

const SKILLS = [
  { name: "Python", keywords: ["python", "pandas", "numpy", "fastapi", "flask", "django"] },
  { name: "SQL", keywords: ["sql", "postgres", "postgresql", "mysql", "sqlite", "snowflake"] },
  { name: "Machine Learning", keywords: ["machine learning", "tensorflow", "pytorch", "scikit", "xgboost"] },
  { name: "React", keywords: ["react", "redux", "hooks", "frontend"] },
  { name: "Node.js", keywords: ["node.js", "nodejs", "express", "nest", "backend"] },
  { name: "Next.js", keywords: ["next.js", "nextjs"] },
  { name: "JavaScript", keywords: ["javascript", "ecmascript"] },
  { name: "TypeScript", keywords: ["typescript"] },
  { name: "AWS", keywords: ["aws", "lambda", "s3", "ec2"] },
  { name: "Docker", keywords: ["docker", "containers", "containerization"] },
...
const QUESTION_BANK = {
  Python: [
    ...
    },
    {
      ...
      options: [
        { id: "a", text: "To lazily produce values and save memory" },
        { id: "b", text: "To sort data automatically" },
        { id: "c", text: "To remove imports" },
        { id: "d", text: "To replace exceptions" },
      ],
      correctOptionId: "a",
  ...
  SQL: [
    ...
    },
    {
      ...
      options: [
        { id: "a", text: "Create an index" },
        { id: "b", text: "Duplicate the table" },
        { id: "c", text: "Drop the primary key" },
        { id: "d", text: "Convert values to images" },
      ],
      correctOptionId: "a",
  ...
  "Machine Learning": [
    {
      ...
      options: [
        { id: "a", text: "Precision-recall tradeoff" },
        { id: "b", text: "Only training accuracy" },
        { id: "c", text: "R-squared" },
        { id: "d", text: "Mean squared error for everything" },
      ],
      correctOptionId: "a",
    ...
    },
    {
      ...
      options: [
        { id: "a", text: "Excellent training results but weak validation performance" },
        { id: "b", text: "Using a train and validation split" },
        { id: "c", text: "Applying regularization" },
  ...
  React: [
    {
      ...
      options: [
        { id: "a", text: "Holding data that can trigger UI updates" },
        { id: "b", text: "Replacing CSS" },
        { id: "c", text: "Persisting files to disk" },
        { id: "d", text: "Eliminating components" },
      ],
      correctOptionId: "a",
    ...
    },
    {
      ...
      options: [
        { id: "a", text: "It helps React reconcile list items predictably" },
        { id: "b", text: "It encrypts props" },
        { id: "c", text: "It removes re-renders completely" },
        { id: "d", text: "It replaces hooks" },
      ],
      correctOptionId: "a",
  ...
  "Node.js": [
    {
      ...
      options: [
        { id: "a", text: "It uses an event-driven non-blocking model" },
        { id: "b", text: "It handles only one request per minute" },
        { id: "c", text: "It replaces databases" },
        { id: "d", text: "It removes network latency" },
      ],
      correctOptionId: "a",
    ...
    },
    {
      ...
      options: [
        { id: "a", text: "A function in the request-response chain" },
        { id: "b", text: "A CSS stylesheet" },
        { id: "c", text: "A database backup" },
        { id: "d", text: "A build artifact" },
      ],
      correctOptionId: "a",
...
function normalizeLevel(value = "") {
  ...
  if (["expert", "principal", "lead"].some((token) => lower.includes(token))) return "Expert";
  if (["advanced", "senior", "strong"].some((token) => lower.includes(token))) return "Advanced";
  if (["intermediate", "proficient", "working"].some((token) => lower.includes(token))) return "Intermediate";
  return "Foundational";
}
...
function roleFit(textLower) {
  if (["machine learning", "llm", "pytorch", "tensorflow"].some((token) => textLower.includes(token))) return "AI / Machine Learning Candidate";
  if (["react", "next.js", "node", "typescript"].some((token) => textLower.includes(token))) return "Full-Stack Product Engineer";
  if (["sql", "analytics", "power bi", "tableau"].some((token) => textLower.includes(token))) return "Data / Analytics Candidate";
  if (["aws", "docker", "cloud", "devops"].some((token) => textLower.includes(token))) return "Cloud / Platform Engineer";
...
function currentTitle(lines, textLower) {
  const titles = ["Software Engineer", "Full Stack Developer", "Frontend Developer", "Backend Developer", "Data Analyst", "Data Scientist", "Machine Learning Engineer", "DevOps Engineer"];
...
function projectLines(lines) {
  return lines
    .filter((line) => /built|developed|designed|led|implemented|launched|created/i.test(line))
    .slice(0, 3)
    .map((line, index) => ({
...
function educationLines(lines) {
  return lines.filter((line) => /bachelor|master|b\.tech|m\.tech|degree|university|college/i.test(line)).slice(0, 2).map((line) => ({ label: clean(line) }));
}
...
function heuristicAnalysis({ resumeText, fileName }) {
  ...
  const foundSkills = uniqueBy(
    SKILLS.filter((skill) => skill.keywords.some((keyword) => lower.includes(keyword))).map((skill) => ({
      name: skill.name,
  ...
  const signals = riskSignals({ text: normalized, foundSkills, personContacts: foundContacts, title });
  ...
  return {
    ...
    skills: foundSkills.length
      ? foundSkills
      : [{ name: "General Technology", level: "Intermediate", evidence: "Resume text was limited, so a broad placeholder skill was created.", confidence: 52 }],
    ...
    analysisMode: hasGroqCredentials() ? "Groq fallback analysis" : "Local fallback analysis",
...
function localQuestions(skillObjects) {
  ...
  for (const skill of skillObjects.slice(0, 3).map((item) => item.name)) {
    ...
    if (bank?.length) {
      result.push(...bank.map((question, index) => normalizeQuestion({ ...question, id: `${slugify(skill)}-${index + 1}` }, result.length + index)));
      continue;
    }
    result.push(
      normalizeQuestion(
        {
          ...
          options: [
            { id: "a", text: "Explaining tradeoffs and real implementation choices" },
            { id: "b", text: "Only recognizing the tool name" },
            { id: "c", text: "Avoiding examples completely" },
            { id: "d", text: "Claiming expertise without proof" },
          ],
          correctOptionId: "a",
...
function mergeAnalysis(fallback, aiAnalysis, resumeText, fileName) {
  ...
  const mergedSignals = uniqueBy(
    [...(Array.isArray(aiAnalysis?.riskSignals) ? aiAnalysis.riskSignals : []), ...(fallback.riskSignals || [])].map((item) => clean(item)).filter(Boolean),
    (item) => item,
  ).slice(0, 4);
  ...
  return {
    ...
    confidenceNotes: uniqueBy(
      [...(Array.isArray(aiAnalysis?.confidenceNotes) ? aiAnalysis.confidenceNotes : []), ...(fallback.confidenceNotes || [])].map((item) => clean(item)).filter(Boolean),
      (item) => item,
    ).slice(0, 4),
...
function localReport({ answers, questions, resumeData }) {
  ...
  return {
    ...
    narrative:
      authenticityVerdict === "Likely Original"
        ? `${resumeData.candidateName} showed strong alignment between claimed expertise and observed answers.`
        : authenticityVerdict === "Needs Deeper Validation"
          ? `${resumeData.candidateName} shows some real signal, but the screening uncovered gaps that need live follow-up.`
          : `${resumeData.candidateName} shows a meaningful mismatch between resume claims and screening answers.`,
    riskSummary: resumeData.riskSignals[0] || "No major pre-interview anomalies were detected from the resume text.",
    chartData: skills.map((skill) => ({ skill: skill.skill, Claimed: LEVEL_SCORE[skill.claimedLevel] ?? 50, Observed: skill.score })),
    signalCards: [
      { label: "Interview Accuracy", value: `${overallScore}%`, tone: overallScore >= 70 ? "good" : overallScore >= 50 ? "warn" : "risk" },
      { label: "Authenticity Confidence", value: `${authenticityScore}%`, tone: authenticityScore >= 70 ? "good" : authenticityScore >= 50 ? "warn" : "risk" },
      { label: "Resume Risk Signals", value: `${resumeData.riskSignals.length}`, tone: resumeData.riskSignals.length <= 1 ? "good" : resumeData.riskSignals.length <= 2 ? "warn" : "risk" },
      { label: "Pipeline", value: resumeData.analysisMode.includes("Groq") ? "Groq" : "Fallback", tone: "neutral" },
    ],
    interviewInsights: [
...
export async function analyzeUploadedResume(file) {
  const fileName = typeof file?.name === "string" ? file.name : "uploaded-resume";
  ...
  const fallback = heuristicAnalysis({ resumeText, fileName });
  let resumeData = fallback;

  if (hasGroqCredentials()) {
    try {
      resumeData = await groqResumeAnalysis({ resumeText, fallback });
    } catch (error) {
      resumeData = { ...fallback, confidenceNotes: [`Groq resume analysis fallback activated: ${error.message}`, ...fallback.confidenceNotes].slice(0, 4) };
    }
  }
  ...
  if (hasGroqCredentials()) {
    try {
      questions = await groqQuestions({ resumeData, resumeText });
    } catch (error) {
      questions = localQuestions(resumeData.skills);
      resumeData = { ...resumeData, confidenceNotes: [`Groq question generation fallback activated: ${error.message}`, ...resumeData.confidenceNotes].slice(0, 4) };
    }
  }

  return { resumeData, questions, resumeText };
}

export async function generateQuestionsForScreening({ skills = [], resumeText = "" }) {
...
export async function evaluateResumeAuthenticity({ answers, questions, resumeData }) {
  const local = localReport({ answers, questions, resumeData });
  if (!hasGroqCredentials()) return local;
  try {
    const ai = await groqEvaluation({ resumeData, local });
    return mergeReport(local, ai);
  } catch (error) {
    return { ...local, interviewInsights: [`Groq authenticity fallback activated: ${error.message}`, ...local.interviewInsights].slice(0, 4) };
  }
}
